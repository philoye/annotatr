...
(function($) {
    ...
    findAnnotations: function(){
      ...
      $('.annotate').each(function(index) {
        ...
        $(this).removeClass("annotate").addClass("annotated").css("position","relative").removeAttr("title").append('<div class="annotation"><div class="annotation-callout">' +(index + 1)+ '</div><div class="annotation-text">'+ annotation_text +'</div');

        $('#page-spec').append('<tr><th>' +(index + 1)+ '</th><td>'+annotation_text+'</td></tr>');
      });
      
      $('.annotation-callout').hover(
