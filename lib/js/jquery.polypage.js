...
(function($) {
	...
	$.polypage = {
		...
		init: function(defaults,selector){
			this.assignDisplayStyles();
			this.buildOptionsBar(selector);
      ...
      $.annotate.buildSpecificationTable();
		},
		...
		buildOptionsBar: function(selector){
		  selector = selector || "body";
			this.findStates();
			var self=this;

			var alphaStates = this.alphabeticalStateNames();
      if (alphaStates.length == 0) { return; }
      $(selector).append('<div id="pp_options"><ul><li class="instructions">Show page state</li></ul></div>');
